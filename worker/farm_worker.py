...
ROLE=os.environ.get('ROLE','UNKNOWN_ROLE'); MISSION=os.environ.get('MISSION','Business and market analysis'); MODEL=os.environ.get('MODEL','huggingface-projects/llama-3.2-3B-Instruct')
prompt=f'''You are CEREBRON Omega Farm 22 role: {ROLE}.\nMission: {MISSION}\nRules: REALITY > COHERENCE; CLAIM <= EVIDENCE; FORECAST != FACT; MARKET SIZE != ADDRESSABLE REVENUE; INTEREST != PURCHASE INTENT; UNIT ECONOMICS BEFORE SCALE; UNKNOWN REMAINS UNKNOWN. Return concise structured analysis with: claims, evidence/assumptions, method, uncertainty, risks, falsification tests, and status labels FACT/DERIVED/ASSUMPTION/FORECAST/SCENARIO/UNKNOWN. Do not pretend external verification occurred unless evidence is supplied.'''
...
with open(f'results/{ROLE}.json','w',encoding='utf-8') as f: json.dump(out,f,ensure_ascii=False,indent=2)
